...
RegisterServerEvent('esx_gold_robbery:toofar')
AddEventHandler('esx_gold_robbery:toofar', function(robb)
	local source = source
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('esx_gold_robbery:killblip', xPlayers[i])
		end
	end
	if(robbers[source])then
		TriggerClientEvent('esx_gold_robbery:toofarlocal', source)
		robbers[source] = nil
		TriggerClientEvent('esx:showNotification', source, _U('robbery_has_cancelled') .. Stores[robb].nameofstore)
	end
end)

RegisterServerEvent('esx_gold_robbery:endrob')
AddEventHandler('esx_gold_robbery:endrob', function(robb)
	local source = source
	local xPlayers = ESX.GetPlayers()
	rob = false
	for i=1, #xPlayers, 1 do
	...
	if(robbers[source])then
		TriggerClientEvent('esx_gold_robbery:robberycomplete', source)
		robbers[source] = nil
		TriggerClientEvent('esx:showNotification', source, _U('robbery_has_ended') .. Stores[robb].nameofstore)
	end
end)

RegisterServerEvent('esx_gold_robbery:rob')
AddEventHandler('esx_gold_robbery:rob', function(robb)
	...
	if Stores[robb] then
		...
		if (os.time() - store.lastrobbed) < 600 and store.lastrobbed ~= 0 then
            ...
            TriggerClientEvent('esx_gold_robbery:togliblip', source)
			...
			if(cops >= Config.RequiredCopsRob)then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
							...
							TriggerClientEvent('esx_gold_robbery:setblip', xPlayers[i], Stores[robb].position)
					end
				end
			    ...
			    TriggerClientEvent('esx_gold_robbery:currentlyrobbing', source, robb)
                CancelEvent()
...
RegisterServerEvent('esx_gold_robbery:gioielli1')
AddEventHandler('esx_gold_robbery:gioielli1', function()
...
local function Craft(source)
	...
	SetTimeout(4000, function()
		...
		if PlayersCrafting[source] == true and CopsConnected >= Config.RequiredCopsSell then
			...
			local xPlayer  = ESX.GetPlayerFromId(source)
			local JewelsQuantity = xPlayer.getInventoryItem('gold').count
